...
window.openAddPartSpecModal = function (site, equip, itemObj, onAddCallback) {
    ...
    confirmBtn.onclick = async () => {
        ...
        const isDuplicate = data.maint.some(m => (m.content === itemName || (m.code && itemObj.code && m.code === itemObj.code)) && (m.spec || '') === newSpec);
        if (isDuplicate) return alert('이미 동일한 물품명과 물품 상세를 가진 항목이 존재합니다.');

        const newItem = { id: `item_${Date.now()}_${Math.floor(Math.random() * 900 + 100)}`, type: '', detailType: 'Parts 교체', code: itemObj.code || '', content: itemName, spec: newSpec, date: '', period: cycle, scheduledDate: null };
